// ============================================
// Lightopedia Server — Thin Slack shell
...
import { PIPELINE_VERSION } from "./agent/loop.js";
...
/**
 * Fetch user context from Slack profile.
 */
async function fetchUserContext(
  ...
  try {
    ...
    logger.warn("Failed to fetch user context", {
      stage: "slack",
      userId,
      error: err,
    });
...
async function fetchThreadHistoryWithFiles(
  ...
  botUserId?: string
): Promise<{ history: ThreadHistoryMessage[]; files: SlackFile[] }> {
  try {
...
async function handleQuestion(
  ...
  // Fetch thread history, files, and user context in parallel
  const [{ history: threadHistory, files: threadFiles }, userContext] = await Promise.all([
    fetchThreadHistoryWithFiles(
      client,
      message.channel,
      threadTs,
      message.ts,
    ...
    fetchUserContext(client, message.user),
  ]);
  ...
  // Handle the question (with user context)
  const { response, requestId, detailedAnswer } = await handleSlackQuestion(
    input,
    threadHistory,
    userContext
  );
...
slackApp.action(/^(feedback_helpful|feedback_not_helpful|show_more_details|submit_escalation|cancel_escalation)$/, async ({ ack, body, client }: { ack: () => Promise<void>; body: unknown; client: unknown }) => {
  await ack();
  ...
  if (actionId === "submit_escalation") {
    try {
      const escalation = JSON.parse(value) as {
        requestId: string;
        title: string;
        requestType: string;
      ...
      if (channelId && threadTs) {
        await (client as SlackClient).chat.postMessage({
          channel: channelId,
          thread_ts: threadTs,
          text: `\u{1F3AB} Escalation submitted: *${escalation.title}*\nType: ${escalation.requestType.replace(/_/g, " ")}\n\n_A team member will follow up._`,
        });
      }

      // Remove escalation buttons from original message
        ...
        const updatedBlocks = originalBlocks.map((block) => {
          if (block.type === "actions" && Array.isArray(block.elements)) {
            const filteredElements = (block.elements as Array<{ action_id?: string }>).filter(
              (el) => el.action_id !== "submit_escalation" && el.action_id !== "cancel_escalation"
            );
      ...
      logger.error("Failed to process escalation", { stage: "slack", error: err });
    }
    return;
  }

  // Handle escalation cancel
  if (actionId === "cancel_escalation") {
    if (channelId && messageTs && originalBlocks) {
      try {
        const updatedBlocks = originalBlocks.map((block) => {
          if (block.type === "actions" && Array.isArray(block.elements)) {
        ...
        logger.error("Failed to cancel escalation", { stage: "slack", error: err });
      }
    }
    return;
  }
...
app.post("/debug/replay", express.json(), async (req, res) => {
  try {
    ...
    const { executeAgenticPipeline } = await import("./agent/loop.js");
    ...
    res.json({
      ...
      answer: {
        summary: result.answer.summary.slice(0, 500),
        confidence: result.answer.confidence,
      },
      evidence: {
        articleCount: result.evidence.articles.length,
        queriesUsed: result.evidence.retrievalMeta.queriesUsed,
      },
...
logger.info("Starting Lightopedia", {
  stage: "startup",
  port: config.port,
  version: "3.0.0",
});
